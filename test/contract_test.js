...
describe("MarketPlace", function () {
  ...
  it("Should post new item for sale and lock the item from reposting", async function () {
    ...
    TX = await marketplace.nftSellPosting("dummyID", minter.address, "0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266", tokenID, BigNumber.from("1000000000000000000"));
  ...
  it("Should sold nft with 10% cut (First Purchase 10% platform fee) and unlock item for posting", async function () {
    ...
    tokenID = MintEvent[0].args[0];
    await minter.setApprovalForAll(marketplace.address, true);
    await marketplace.nftSellPosting("dummyID", minter.address, "0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266", tokenID, BigNumber.from("1000000000000000000"));
    ...
    const locked = await marketplace.isItemLocked(tokenID, minter.address);
    expect(NFTSaleEvent[0].args[6]).equals("An NFT from marketplace has been sold");
  ...
  it("Should sold nft with 5% cut (2.5% platform fee + 2.5% royalty)", async function () {
    ...
    await marketplace.nftSellPosting("dummyID", minter.address, buyerAddress.address, tokenID, BigNumber.from("1000000000000000000"));
